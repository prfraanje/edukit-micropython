...
```
On Windows you may want to remove the options `--user` so that the `rshell` is directly available at the command-line in `cmd` (Note, you may need to add the `..\Python312\Scripts\` directory to your path, for help on that, see e.g. [StackOverflow on how to add a folder to path environment variable in windows](https://stackoverflow.com/questions/44272416/how-to-add-a-folder-to-path-environment-variable-in-windows-10-with-screensho)).
