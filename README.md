...
```
and change `collections.Callable` on line 8 into `collections.abs.Callable`. Also see the [post](https://stackoverflow.com/questions/69515086/error-attributeerror-collections-has-no-attribute-callable-using-beautifu) on StackOverflow.
...
- After that, the command line or from your Python IDE run the code `edukit_pc.py.
- If all installations and configurations went fine, you should see a `matplotlib` figure with three axes that show plots of running signals, the first one of the stepper motor microsteps value, the second one of the pendulum encoder value and the last one shows the control signal, that is proportional to the PWM frequency driving the pulses to microstep the stepper-motor.
- Also, the standard Python prompt is replaced by an alternative REPL (Read Eval Print Loop), indicated by `-> `, in which you can enter Python code that will be evaluated on the PC, but also you can, by prefixing your code with `mp `, give enter code to be run by the micropyton on the microcontroller:
```
# run on PC:
from math import *
atan2(4,3)
